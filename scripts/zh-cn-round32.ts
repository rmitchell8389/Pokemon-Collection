// Round 32 — continuing down the CSV (Scarlet & Violet) chain past round
// 31's CSV9.5C. Same track as rounds 13-31 (see src/lib/cnBoosterSetImport.ts
// and claude/spec.md for the full mechanism/history).
//
// This round's target is NOT a guess — it came directly from CSV9.5C's own
// {{ExpansionPrevNext}} chain reference, captured byte-for-byte in
// data/wiki-raw/terastal-gathering-round31-raw.txt:
//   `prev=星彩晶璃, next=共逐荣光, other=, other2=`
// (no `other` side branch this round — single-strand chain.)
//
//   "共逐荣光（TCG）" — plausibly CSV10C, the actual last of the
// originally-named CSV1C-CSV10C family (CSV9.5C turned out to be a bridge
// set, not CSV10C itself — see round 31's findings). Genuinely unconfirmed
// until fetched.
//
...
//
// Usage: npx tsx scripts/zh-cn-round32.ts > round32-raw.txt
// Then send round32-raw.txt back (paste the content or attach the file).

import { fetchFullContent, sleep } from "../src/lib/cnReprintImport";

const TITLES = [
  "共逐荣光（TCG）", // named directly by CSV9.5C's own chain ref (next=)
];
