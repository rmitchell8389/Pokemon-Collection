// Round 29 — continuing down the CSV (Scarlet & Violet) chain past round
// 28's CSV7C. Same track as rounds 13-28 (see src/lib/cnBoosterSetImport.ts
// and claude/spec.md for the full mechanism/history).
//
// This round's target is NOT a guess — it came directly from CSV7C's own
// {{ExpansionPrevNext}} chain reference, captured byte-for-byte in
// data/wiki-raw/blade-awakening-round28-raw.txt:
//   `prev=真实玄虚, next=璀璨诡幻, other=, other2=`
// (no `other` side branch this round — single-strand chain.)
//
//   "璀璨诡幻（TCG）" — plausibly CSV8C. Genuinely unconfirmed until fetched.
//
...
//
// Usage: npx tsx scripts/zh-cn-round29.ts > round29-raw.txt
// Then send round29-raw.txt back (paste the content or attach the file).

import { fetchFullContent, sleep } from "../src/lib/cnReprintImport";

const TITLES = [
  "璀璨诡幻（TCG）", // named directly by CSV7C's own chain ref (next=)
];
