// Backfills `cards.image_url` for vintage (pre-2011) Japanese cards using
// pcg-search.com as a fallback image source — the counterpart to
// scripts/backfill-images-jp.ts (which covers modern Japanese sets via
// limitlesstcg.com, a source that doesn't reach back this far). See
// src/lib/pcgsearch.ts for how the URL scheme was found and verified.
//
// Usage:
//   npm run backfill:images-jp-vintage
//
...
// trusted; nothing is written on a guess. Cards whose set_id doesn't parse
// as "<letters><digits>" (parseVintageSetId returning null) are skipped
...
import { buildPcgSearchJpImageUrl, pcgSearchImageExists } from "../src/lib/pcgsearch";
...
async function main() {
  ...
  for (const [setId, cards] of setEntries) {
    ...
    await mapWithConcurrency(cards, CONCURRENCY, async (card) => {
      const candidateUrl = buildPcgSearchJpImageUrl(setId, card.card_number);
      ...
      const exists = await pcgSearchImageExists(candidateUrl);
      if (!exists) {
        totalNotFound++;
        return;
      }
  ...
  console.log(`Not found on pcg-search (real 404, checked per-card): ${totalNotFound}`);
  if (totalUnparseable > 0) {
    console.log(`Set id didn't match the expected "<letters><digits>" shape, skipped: ${totalUnparseable}`);
  }
  if (totalUpdateFailed > 0) {
    console.log(`Database update failed for: ${totalUpdateFailed}`);
  }
  console.log(
    "Anything still missing after this genuinely isn't on pcg-search either right now — a real gap, not a bug (VS1 and web1 in particular weren't confirmed to resolve at all, see src/lib/pcgsearch.ts)."
  );
}
