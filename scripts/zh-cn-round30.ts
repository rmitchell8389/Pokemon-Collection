// Round 30 — continuing down the CSV (Scarlet & Violet) chain past round
// 29's CSV8C. Same track as rounds 13-29 (see src/lib/cnBoosterSetImport.ts
// and claude/spec.md for the full mechanism/history).
//
// This round's target is NOT a guess — it came directly from CSV8C's own
// {{ExpansionPrevNext}} chain reference, captured byte-for-byte in
// data/wiki-raw/sparkling-fable-round29-raw.txt:
//   `prev=利刃猛醒, next=星彩晶璃, other=, other2=`
// (no `other` side branch this round — single-strand chain.)
//
//   "星彩晶璃（TCG）" — plausibly CSV9C. Genuinely unconfirmed until fetched.
//
...
//
// Usage: npx tsx scripts/zh-cn-round30.ts > round30-raw.txt
// Then send round30-raw.txt back (paste the content or attach the file).

import { fetchFullContent, sleep } from "../src/lib/cnReprintImport";

const TITLES = [
  "星彩晶璃（TCG）", // named directly by CSV8C's own chain ref (next=)
];
